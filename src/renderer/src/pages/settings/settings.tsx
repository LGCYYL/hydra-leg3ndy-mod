...
import { SettingsContextBackup } from "./settings-context-backup";
...
export default function Settings() {
  ...
  const categories = useMemo(
    () => [
      ...
      {
        id: "debrid" as const,
        label: t("debrid", { defaultValue: "Debrid" }),
        icon: <CloudIcon size={16} />,
      },
      {
        id: "backup" as const,
      ...
      <SettingsContextConsumer>
        {({ currentCategoryId, setCurrentCategoryId, appearance }) => {
          ...
          const renderCategory = () => {
            ...
            if (selectedCategoryId === "debrid") {
              return <SettingsDebrid />;
            }

            if (selectedCategoryId === "backup") {
              return <SettingsContextBackup />;
            }
                ...
                <aside className="settings__sidebar">
                  {categories.map((category) => (
                    <button
                      key={category.id}
                      type="button"
                      className={`settings__sidebar-button ${currentCategory.id === category.id
